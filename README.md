...
---
*계층형으로 설명페이지 작성하니깐 훨씬 깔끔함 - 위에도 이렇게 통일하자
...
- 상호작용
  ...
  - 효과음 : 타이머 시작과 종료시 서로 상반되는 효과음을 출력한다
- 버튼 위치(이름바꿀까?)
  - 홈버튼 : 좌측상단 & 네비게이션 버튼 2가지로 구현
    -  이유? : UX를 위한 홈버튼(좌측상단), UI를 위한 홈버튼(네비게이션 바)
    -  -> 리스트로 만들고 간단하게 설명하기
  - 로그인버튼 : 우측상단
    - 다른 웹 서비스들을 이용했을 때 내가 익숙했던 위치에 로그인을 위한 버튼을 배치 
